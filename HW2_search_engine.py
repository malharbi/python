...
query=input("query:")           #user enter for search in specific data in (data_list)
lookFor=query.split()           #cconvert the string to list
lookFor=list(set(lookFor))      #to delete the duplicate string from the list to be unique
# here three situation: the query has both "or" and "and", just "and" or nither "and" nor "or"
if "and" in lookFor and "or" in lookFor or "and" and "or" not in lookFor or "and" and "or" not in lookFor:
        if "and" in lookFor:
                lookFor.remove("and")
        if "or" in lookFor:
                lookFor.remove("or")
        for i,quote in enumerate(data_list):
                ...
                for word in lookFor:
                        if word in quote:
                                ...
                                if found_at < startQuote:
                                        startQuote= found_at # to print the whole sentence that contain all words
                if count==len(lookFor):
                        print("Found at:",i,"..."+ quote[startQuote:found_at+200],"...")
# this situation for OR
elif "or" in lookFor:
        lookFor.remove("or")
        print(lookFor,"the situation as OR ")
        for quote in data_list:
                for word in lookFor:
                        found_at =quote.find(word)
                        if found_at >=0:
                                print("Found:", "..."+quote[found_at:found_at+100], "...")
else:
        print("Nothing match for:",query)
